...
/**
 * <b>PrefixSpanTarget</b> - Static batch baseline that adapts the
 * classical PrefixSpan projected-database algorithm to the target-SPM
 * setting.
 *
 * <p>PrefixSpan mines sequential patterns by successively projecting
 * the database along growing prefixes; only occurrences that follow
 * the current prefix are retained in each projected DB. Here we bolt
 * on a target-item filter: a pattern is emitted only if it contains at
 * least one item from {@link AppConfig#TARGET_ITEMS}, but the
 * projection itself is unchanged.
 *
 * <p>Like {@link BatchTaSPM}, this baseline reloads the entire prefix
 * of the dataset that has arrived so far on every batch, and the
 * reported wall time deliberately includes that I/O so the paper can
 * fairly demonstrate the cost of a static miner against
 * Ada-IncTaSPM's incremental approach.
 */
public class PrefixSpanTarget {
    ...
    private static class RawSequence {
        int sidGoc;
        List<int[]> itemsets;
        RawSequence(int sidGoc) { this.sidGoc = sidGoc; this.itemsets = new ArrayList<>(); }
    }

    private static class ProjectedSequence {
        int sidIdx, tidIdx, itemOffset;
        ProjectedSequence(int sidIdx, int tidIdx, int itemOffset) {
            this.sidIdx = sidIdx; this.tidIdx = tidIdx; this.itemOffset = itemOffset;
        }
    }
    ...
    private final String OUTPUT_FILE = "result_PrefixSpanTarget.txt";
    ...
    // ========================================================================
    // 3. ORCHESTRATION (PURE BATCH)
    // ========================================================================
    public void run() {
        AppConfig.load();
        System.out.println("======================================================");
        System.out.println("=== PREFIXSPAN-TARGET (PURE BATCH W/ I/O METRICS)  ===");
        System.out.println("======================================================");

        // 1. Quick scan of the file to count lines
        ...
        // --- PHASE 1: MEASURE ALGORITHM PERFORMANCE ---
        for (int i = 1; i <= AppConfig.TOTAL_RUNS; i++) {
            ...
            // --- PHASE 2: WRITE RESULTS TO DISK AND PRINT SUMMARY ---
            if (isLastRun) {
                try (BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(OUTPUT_FILE), StandardCharsets.UTF_8))) {
                    ...
                    bw.write("- Algorithm parameters: PrefixSpan (Pure Batch - includes I/O cost)\n");
                    ...
                    System.out.println("[OK] File output complete: result_PrefixSpanTarget.txt!");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        ...
        for (RawSequence seq : dataset) {
            ...
            for (int[] itemset : seq.itemsets) {
                for (int item : itemset) {
                    if (AppConfig.TARGET_ITEMS.contains(item)) { hasTarget = true; break; }
                }
                if (hasTarget) break;
            }
    ...
    public static void main(String[] args) { new PrefixSpanTarget().run(); }
}